...
# Gốc dự án suy ra từ vị trí FILE này, KHÔNG phụ thuộc thư mục đang chạy (cwd).
# Cần thiết vì Streamlit Cloud chạy app với cwd = gốc repository, trong khi mã
# nguồn và dữ liệu nằm trong một thư mục con — dùng đường dẫn tương đối theo cwd
# sẽ không tìm thấy data/processed.
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_PROCESSED_DIR = PROJECT_ROOT / "data" / "processed"


def load_processed_split(
    source: str, split: str, processed_dir: str | Path | None = None
) -> pd.DataFrame:
    ...
    """
    processed_dir = DEFAULT_PROCESSED_DIR if processed_dir is None else Path(processed_dir)
...
def list_available_sources(processed_dir: str | Path | None = None) -> list[str]:
